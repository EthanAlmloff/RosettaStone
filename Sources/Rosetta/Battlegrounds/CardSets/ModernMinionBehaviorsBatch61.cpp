...
void ModernMinionBehaviorsBatch61::AddAll(std::map<std::string, CardDef>& cards) {
  ...
  played.SetTriggerSource(TriggerSource::MINIONS_EXCEPT_SELF);
  played.SetCondition(SelfCondition{[](Minion& source) {
  ...
  goldenPlayed.SetTriggerSource(TriggerSource::MINIONS_EXCEPT_SELF);
  goldenPlayed.SetCondition(SelfCondition{[](Minion& source) {
    return source.HasRace(Race::ELEMENTAL);
  }});
