...
TaskStatus BuyTavernSpellMurlocTask::Run(Player& player, int& progress) {
  if (m_limit > 0 && progress >= m_limit) return TaskStatus::COMPLETE;
  if (player.lastBoughtTavernSpellID.empty() || player.hand.IsFull())
