...
TaskStatus StartCombatHighestHandMurlocSummonTask::Run(Player& p, Minion&) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
        if (!p.GetField().IsFull()) {
            Minion copy{*candidates[i]}; p.ApplyFreshMinionModifiers(copy); copy.getPlayerCallback = [&p]() -> Player& { return p; }; if (p.getNextCardIndexCallback) copy.SetIndex(p.getNextCardIndexCallback()); p.GetField().Add(copy, p.GetField().GetCount());
