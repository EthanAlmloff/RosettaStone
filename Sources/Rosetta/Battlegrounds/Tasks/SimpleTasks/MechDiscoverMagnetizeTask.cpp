...
TaskStatus MechDiscoverMagnetizeTask::Run(Player& p, Minion& source, Minion& target) {
 ...
 std::vector<Card> candidates; for(const auto& c:Cards::GetAllCards()) if(c.isBattlegroundsPoolMinion&&c.normalDbfID==0&&HasActiveTribe(p.activeTribes,c)&&c.hasBehavior&&c.GetCardType()==CardType::MINION&&c.HasRace(Race::MECHANICAL)&&c.gameTags.contains(GameTag::MAGNETIC)&&c.gameTags.at(GameTag::MAGNETIC)!=0) candidates.push_back(c);
