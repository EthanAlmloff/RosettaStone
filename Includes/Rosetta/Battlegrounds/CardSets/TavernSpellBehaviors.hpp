...
enum class TavernSpellEffect
{
    ...
    //! Titus' Tribute: repeat every friendly Deathrattle while armed.
    TEMPORARY_DEATHRATTLE_REPEAT,
    //! Primal Staff: repeat end-of-turn effects in the current recruit turn.
    TEMPORARY_END_TURN_REPEAT,
};
...
inline TavernSpellBehavior FindTavernSpellBehavior(std::string_view id)
{
    ...
    if (id == "BG28_814")
        return { 0, 0, 0, TavernSpellEffect::TARGET_END_TURN_STATS };
    // These player enchantments are intentionally not marked as pool spells:
    // they are generated by Trinket/portrait paths.  Their parent IDs remain
    // explicit so card-data-only enchantments cannot become executable.
    if (id == "BG28_843") // Titus' Tribute: Deathrattles twice until next turn.
        return { 0, 0, 0, TavernSpellEffect::TEMPORARY_DEATHRATTLE_REPEAT };
    if (id == "BG28_955") // Primal Staff: end-of-turn effects twice this turn.
        return { 0, 0, 0, TavernSpellEffect::TEMPORARY_END_TURN_REPEAT };
...
inline bool TavernSpellRequiresTarget(TavernSpellEffect effect) noexcept
{
    return effect == TavernSpellEffect::TARGET_STATS ||
           ...
           effect == TavernSpellEffect::TARGET_END_TURN_STATS ||
