    ...
    Season14HeroPowerBatch4Result& result) noexcept
{
    ...
    if (event == Season14HeroPowerBatch4Event::PLAY_MINION && dbfID == 61917)
    {
        ...
        {
            // Verdant Spheres buffs the minion at the successful play
            // boundary.  The Player caller applies this result to the fresh
            // instance before it enters the recruit field.
            result.attack = 2;
            result.health = 2;
            state.minionsPlayedThisTurn = 0;
        }
