...
class Season14State
{
 public:
    ...
    //! Jewelry Box reward not yet opened because hand/modal state was busy.
    //! The counter is consumed only after the typed Blood Gem choice commits.
    std::int32_t pendingJewelryBoxBloodGemRemaining = 0;
    //! Innkeeper's Hearth golden token keeps its second Tier-6 Discover
    //! pending until the first selection has been committed.
    std::int32_t pendingInnkeepersHearthRemaining = 0;
    //! The first golden Hearth selection is consumed from the second pool so
    //! the two generated minions are distinct.  Zero means no selection has
    //! committed for the current golden token.
    std::int32_t pendingInnkeepersHearthSelectedDbfID = 0;
    ...
    //! Owned Trinket slot being replaced by a Mystery Cube modal.  This is
    //! set only while the two-offer public modal is pending.
    std::int32_t pendingTrinketReplacementSlot = -1;
    //! Mystery Cube's start-turn offer was blocked by another public modal.
    //! Keep this separate from the modal payload so resolving that modal can
    //! safely retry without replacing or recursively opening a choice.
    bool pendingMysteryCubeOffer = false;
    //! Trip Vouchers' scheduled Greater-Trinket replacement was blocked by
    //! another public modal and must be retried at the next safe boundary.
    bool pendingTripVouchersOffer = false;
    //! Tickatus acquisition Discover was blocked by another public modal.
    bool pendingTickatusDiscover = false;
    ...
    //! Soul Fermenter owns exact combat-only snapshots until its delayed
    //! resummon has been committed.  These are instance snapshots, not card
    //! IDs, so enchantments and golden state survive the destroy boundary.
    std::vector<Minion> soulFermenterSnapshots;
    bool soulFermenterArmed = false;
    ...
    // Number of recruit starts for which a Discover-created hand minion is
    // locked. Zero keeps legacy one-turn lock semantics.
    std::int32_t pendingHandLockTurns = 0;
    ...
    //! Keep each random-refresh spell application independent.  The legacy
    //! scalar totals remain for compatibility with older callers, but a
    //! spell cast four times must be able to choose four random offers.
    std::vector<std::pair<std::int32_t, std::int32_t>>
        refreshRandomShopStatApplications;
    ...
    std::int32_t temporaryBloodGemAttackBonus = 0;
    std::int32_t temporaryBloodGemHealthBonus = 0;
    ...
    void AddTemporaryBloodGemBonus(std::int32_t attack, std::int32_t health) noexcept;
    void ResetTemporaryBloodGemBonus() noexcept;
    ...
    //! Consume one independent random-offer payload per armed spell cast.
    std::vector<std::pair<std::int32_t, std::int32_t>>
    ConsumeRefreshRandomShopStatApplications() noexcept;
